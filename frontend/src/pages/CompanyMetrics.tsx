...
export default function CompanyMetrics() {
  ...
  if (loading) return <p className="p-6">Loading company data…</p>;

  return (
    <div className="p-6 space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">{form.name?.trim() || "Company"}</h1>
        <button type="button" className="border rounded px-3 py-1" onClick={loadAll} disabled={eqLoading}>
          {eqLoading ? "Loading…" : "Reload"}
        </button>
      </div>
      ...
      {note && (
        <div
          className={`rounded-md border p-3 text-sm ${
            note.type === "ok" ? "border-green-300 text-green-700" : "border-red-300 text-red-700"
          }`}
        >
          {note.text}
        </div>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="pt-2">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-semibold">Company Metrics</h2>
          </div>

          <form onSubmit={onSave} className="space-y-6 border rounded p-4 bg-gray-50">
            {/* Row 1 */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {/* Current Market Value */}
              <div>
                <label className="block text-sm mb-1">Current Market Value</label>
              ...
              <div>
                <div className="flex items-center justify-between">
                  <label className="block text-sm mb-1">Total Authorized Shares</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {/* FMV */}
              <div>
                <label className="block text-sm mb-1">FMV</label>
                <div className="relative">
                  <input
                    className="w-full border rounded pl-10 pr-3 py-2"
              ...
              {/* Volatility */}
              <div>
                <label className="block text-sm mb-1">Volatility</label>
                <div className="relative">
                  <input
                    className="w-full border rounded pr-10 px-3 py-2"
              ...
              {/* Risk Free Rate */}
              <div>
                <label className="block text-sm mb-1">Risk Free Rate</label>
                <div className="relative">
                  <input
                    className="w-full border rounded pr-10 px-3 py-2"
      ...
      <section className="border rounded bg-white">
        ...
        <div
          className={`overflow-hidden transition-[max-height] duration-300 ${openSeries ? "max-h-[2000px]" : "max-h-0"}`}
        >
          <div className="px-4 pb-4">
            <div className="flex items-center justify-end mb-2">
              <button type="button" className="border rounded px-3 py-1" onClick={() => setShowSeriesModal(true)}>
                + Add Series
              </button>
            </div>
            ...
            <div className="space-y-3">
              {series.length === 0 && (
                <div className="text-gray-500 text-sm border rounded p-3">No series configured. Click “Add Series”.</div>
              )}

              {series.map((s) => (
                <div key={String(s.id)} className="grid grid-cols-12 gap-2 items-end border rounded p-3">
      ...
      <section className="border rounded bg-white">
        ...
        <div
          className={`overflow-hidden transition-[max-height] duration-300 ${openClasses ? "max-h-[2000px]" : "max-h-0"}`}
        >
          <div className="px-4 pb-4">
            <div className="flex items-center justify-end mb-2">
              <button type="button" className="border rounded px-3 py-1" onClick={() => setShowClassModal(true)}>
                + Add Class
              </button>
            </div>
            ...
            <div className="space-y-3">
              {classes.length === 0 && (
                <div className="text-gray-500 text-sm border rounded p-3">No classes configured. Click “Add Class”.</div>
              )}

              {classes.map((c) => (
                <div key={String(c.id)} className="grid grid-cols-12 gap-2 items-end border rounded p-3">
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 w-full max-w-md">
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button className="border rounded px-3 py-1" onClick={() => setShowClassModal(false)}>
                Cancel
              </button>
              <button className="px-3 py-1 rounded bg-black text-white" onClick={createClass}>
                Add
              </button>
            </div>
          </div>
      ...
      {showSeriesModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 w-full max-w-md">
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button className="border rounded px-3 py-1" onClick={() => setShowSeriesModal(false)}>
                Cancel
              </button>
              <button className="px-3 py-1 rounded bg-black text-white" onClick={createSeries}>
                Add
              </button>
            </div>
          </div>
